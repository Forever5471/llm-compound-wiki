...
The CLI does not browse the live web by itself. Give the generated `web-query-*.md` prompt to a browser-capable agent that can use `wiki-agent-browser`; that agent should search, open sources, write `.cwiki/web-research/*.md`, and then answer from the generated fusion prompt.
...
The `.env` model settings control terminal/CLI workflows that the project itself runs, such as `cwiki answer`. They do not change the model selected by Trae, Codex, Claude Code, Cursor, or another agent platform for the current chat.
