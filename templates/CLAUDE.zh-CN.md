...
- `link-graph` 和 `link-graph-report` 从已编译 wiki 页面与 `[[wikilink]]` 创建确定性 link graph；`graph` 和 `graph-report` 是兼容别名。
...
- `answer --web-on-gaps` 和 `eval-answer --web-on-gaps` 会识别需要外部证据的缺口，并生成 `.cwiki/web-gaps/`、`web-query-*`、`web-research-*`、`web-captures-*`、`fusion-*` 产物。默认读取同一组 `CWIKI_WEB_*`，除非用 `--web-gap-*` 单次覆盖。
...
2. 读取生成的 browser prompt、web capture checklist 和 fusion prompt。
3. 先读本地 wiki 上下文，再浏览。
4. 搜索并打开来源页面；不要引用搜索结果摘要。
5. 将 web findings 写入 `.cwiki/web-research/*.md`。
6. 同步更新 `.cwiki/web-captures/*.md`，记录哪些来源需要进入 `raw/captures/`。
7. 最终答案分开写 local wiki evidence、web evidence、synthesis、gaps、sources。
8. 重要 web 来源先 `cwiki capture`，再摄入 `wiki/`。
