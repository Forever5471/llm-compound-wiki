...
class CwikiTest(unittest.TestCase):
    def test_init_creates_a_usable_wiki(self) -> None:
        with tempfile.TemporaryDirectory(prefix="cwiki-") as tmp:
            ...
            self.assertTrue((root / "CLAUDE.zh-CN.md").exists())
            ...
            claude_zh = (root / "CLAUDE.zh-CN.md").read_text(encoding="utf-8")
            self.assertIn("领域：Test knowledge", claude_zh)
            self.assertIn("分层检索策略", claude_zh)
            self.assertIn("Agent 平台混合问答流程", claude_zh)
            self.assertIn("--agent-eval-file", claude_zh)
            self.assertIn("Graph-Expanded Pages", claude_zh)
            ...
            self.assertIn("分层检索策略", agents_zh)
            self.assertIn("Agent 平台混合问答流程", agents_zh)
            self.assertIn("--retrieval auto", agents_zh)
            self.assertIn("--agent-eval-file", agents_zh)
