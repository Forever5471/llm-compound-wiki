...
Add `--web-on-gaps` when evaluation should turn external-evidence gaps into a follow-up workflow. The evaluator scans the answer `## Gaps`, deterministic warnings, and any attached LLM-assisted evaluation for missing case studies, metrics, current facts, or migration guidance. If triggered, it creates `.cwiki/web-gaps/`, web query, web research, and fusion prompt artifacts.
